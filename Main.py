...
class TooltipHelper(QtCore.QObject):
    def __init__(self, list_widget, parent_gui):
        super().__init__()
    ...
    def eventFilter(self, obj, event):
        if obj == self.list_widget.viewport() and event.type() == QtCore.QEvent.MouseMove:
            item = self.list_widget.itemAt(event.pos())
            if item:
                macro_file = item.text()
                info_text = self.parent_gui.get_macro_info(macro_file)
                if info_text and not macro_file.startswith("No macros found"):
                    QtWidgets.QToolTip.showText(event.globalPos(), info_text, self.list_widget)
                else:
                    QtWidgets.QToolTip.hideText()
            else:
                QtWidgets.QToolTip.hideText()
        if obj == self.list_widget.viewport() and event.type() == QtCore.QEvent.Leave:
            QtWidgets.QToolTip.hideText()
...
class MacroPlayerGUI(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.macro_listbox.setSpacing(0)
        self.macro_listbox.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        accent = get_theme_accent(os.path.join(THEME_DIR, self.settings.get("theme", "dark_teal.xml")))
        self.macro_listbox.setStyleSheet(f"""
...
""")
        self.tooltip = TooltipHelper(self.macro_listbox, self)
    ...
    @QtCore.pyqtSlot()
    def play_macro(self):
        selected_items = self.macro_listbox.selectedItems()
        ...
        if self.macro_running and not self.macro_paused:
            return
        if selected_items and not selected_items[0].text().startswith("No macros found"):
            macro_file = selected_items[0].text()
    ...
    def reload_macros(self):
        self.macro_listbox.clear()
        ensure_macro_folder()
        macros = [f for f in os.listdir(MACRO_FOLDER) if f.endswith(".json")]
        if macros:
            self.macro_listbox.addItems(macros)
        else:
            self.macro_listbox.addItem("No macros found. Put .json macro files in the 'macros' folder.")
    ...
    def get_macro_info(self, macro_file):
        macro_path = os.path.join(MACRO_FOLDER, macro_file)
        if macro_file.endswith(".json") and os.path.exists(macro_path):
            try:
                with open(macro_path, "r", encoding="utf-8") as f:
                    actions = json.load(f)
